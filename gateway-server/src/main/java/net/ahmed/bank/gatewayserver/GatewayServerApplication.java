...
public class GatewayServerApplication {
		...
		return routeLocatorBuilder.routes()
				...
				.route(p ->  p
						...
						.filters(f -> f.rewritePath("/ahmedbank/loans/(?<segment>.*)","/${segment}")
								...
								.retry(retryConf -> retryConf //retry pattern -> retry to connect to loans microservice
										.setRetries(3)
    ...
    @Bean
    public RedisRateLimiter redisRateLimiter() { // redis rateLimiter config beans
        return new RedisRateLimiter(1, 1, 1);
    }

    @Bean
    KeyResolver userKeyResolver() { // redis rateLimiter config beans
        return exchange -> Mono.justOrEmpty(exchange.getRequest().getHeaders().getFirst("user" )) //user value used as demo instead of real uder name
